var search = (function () {
    ...
    /* Penser Ã  supprimer toutes les rÃ©fÃ©rences Ã  iconsearch, propriÃ©tÃ© plus utilisÃ©e.*/
    ...
    var _initSearch = function () {
        ...
        if (_searchparams.features || _searchparams.static) {
            ...
            var vector = new ol.layer.Vector({
                source: _sourceEls,
                style: app.featureStyles.elsStyle
            });
            vector.set('appid', 'elasticsearch');
        ...
        app.zoomToFeature = search.zoomToFeature;
        app.showFeature = search.showFeature;
    };
            ...
            if (_olsCompletionType === "geoportail") { // Open LS search
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var str = '<a class="geoportail list-group-item disabled">LocalitÃ©s</a>';
                        //var str = '';
                        for (var i = 0, len = res.length; i < len && i < 5; i++) {
                            ...
                            str += '<a class="geoportail list-group-item" href="#" onclick="app.zoomToLocation(' +
                            res[i].x + ',' + res[i].y + ',' + zoom + ',' + _searchparams.querymaponclick +');" '+
                            'onmouseover="app.flash('+'\'EPSG:4326\',' + res[i].x + ',' + res[i].y + ');"> ' +
                            res[i].fulltext + '</a>';
                        }
                        $(".geoportail").remove()
                        if (res.length > 0) {
    ...
    var _sendFuseRequest = function (val) {
        ...
        for (var i = 0; i < searchableLayers.length; i++) {
            ...
            var layerid = searchableLayers[i].get('appid');
            ...
            if (results.length > 0) {
                ...
                results.forEach(function(element){
                  ...
                  element = element.item;
                    ...
                    var xyz = app.getLonLatZfromGeometry(geom, 'EPSG:4326', zoom);
                    str += '<a class="fuse list-group-item" title="' + result_label + '" ' +
                        'href="#" onclick="app.zoomToLocation('
                        + xyz.lon + ',' + xyz.lat + ',' + xyz.zoom + ',' + _searchparams.querymaponclick +');app.showLocation(\'EPSG:4326\','
                        + xyz.lon + ',' + xyz.lat +');" '
                        + 'onmouseover="app.flash(\'EPSG:4326\',' + xyz.lon + ',' + xyz.lat + ');" >'
                        + result_label + '</a>';
                });
            }
            _showResults(str);
        ...
        if (searchableLayers.length > 0 || (_searchparams.static && _elasticSearchDocTypes)) {
            ...
            if (sendQuery) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        _sourceEls.clear();
                        var str = '<a class="elasticsearch list-group-item disabled" >EntitÃ©s</a>';
                        ...
                        for (var i = 0, nb ; i < nb && i < 5; i++) {
                            ...
                            if (geomtype !== 'Point') {
                                ...
                                action_click = 'app.zoomToFeature(\'feature.'+i+'\');';
                                feature.setId( "feature." +i);
                                _sourceEls.addFeature(feature);
                                action_over = 'app.showFeature(\'feature.'+i+'\');';
                            } else {
                                action_click = 'app.zoomToLocation('  + point[0] + ',' + point[1]  + ',14,' + _searchparams.querymaponclick +');';
                                action_over = 'app.flash('+'\'EPSG:4326\',' + point[0] + ',' + point[1] + ');';
                            }
                            if  (_overLayers[data.hits.hits[i]._type] ) {
                                //icon = _overLayers[data.hits.hits[i]._type].iconsearch;
                                action_click += 'app.tools.info.queryLayer(' + point[0] + ',' + point[1] + ',\'EPSG:4326\',\''+
                    ...
                    error: function (xhr, ajaxOptions, thrownError) {
                        app.alert("ProblÃ¨me avec l'instance Elasticsearch.\n" +  thrownError + "\n DÃ©sactivation du service.", "alert-warning");
    ...
    /* appeler cette fonction Ã  partir de init de app aprÃ¨s
    ...
    var _init = function (configuration) {
        _map = app.getMap();
        _sourceOverlay = app.getSourceOverlay();
        _projection = app.getProjection();
        _overLayers = app.getLayers();
